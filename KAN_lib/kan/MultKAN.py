...
class MultKAN(nn.Module):
    ...
    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1.,start_grid_update_step=-1, stop_grid_update_step=50, batch=-1,
        ...
        # make training and testing loss vectors
        training_loss = []
...
class KAN_me(nn.Module):
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.model = MultKAN(*args, **kwargs)
    ...
    def apply_symbolic_rule(self):
        if hasattr(self.model, "symbolic_fun"):
            for layer in self.model.symbolic_fun:
                if hasattr(layer, "apply_symbolic_rule"):
                    layer.apply_symbolic_rule()
    ...
    def symbolic_weight(self):
        try:
            return self.model.symbolic_weight  # If it's a tensor or float
        except AttributeError:
            return None

    @property
    def layers(self):
        return getattr(self.model, "layers", "layers not available (call fit or run forward pass first)")
