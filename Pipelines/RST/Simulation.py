...
def main(config):
   ...
   # Save config.yaml inside the output_dir
    ...
    data_bundle = load_norm_kde(config, exp_dir=exp_dir, rans_dir=rans_dir, save=config['features']['save_kde'], save_path= output_dir/'KDE')
    ...
    best_results = organize_rst_results(y_pred, y_test,case_names=test_cases, config=config)
    fin_results = organize_rst_results(fin_pred, y_test,case_names=test_cases, config=config)
